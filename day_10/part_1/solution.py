...
def main():
    with open(filename, "r") as f:
        ...
        while current_position != starting_point or step_counter == 0:
            ...
            print("Symbol:", maze[current_position[0]][current_position[1]])
            step_counter += 1
        ...
        print(f"The farthest point from the starting position is {(step_counter // 2) + 1} away.")
...
def find_possible_positions(maze_position: tuple, symbol) -> list:
    match symbol:
        case "F":
            return [(maze_position[0], maze_position[1]+1), (maze_position[0]+1, maze_position[1])]
        case "7":
            return [(maze_position[0], maze_position[1]-1), (maze_position[0]+1, maze_position[1])]
        case "J":
            return [(maze_position[0]-1, maze_position[1]), (maze_position[0], maze_position[1]-1)]
        case "L":
            return [(maze_position[0], maze_position[1]+1), (maze_position[0]-1, maze_position[1])]
        case "|":
            return [(maze_position[0]-1, maze_position[1]), (maze_position[0]+1, maze_position[1])]
        case "-":
            return [(maze_position[0], maze_position[1]-1), (maze_position[0], maze_position[1]+1)]
        case default:
            exit("Impossible symbol found!!!")

def find_next_position(previous_position: tuple, maze_position: tuple, symbol: str) -> tuple:
