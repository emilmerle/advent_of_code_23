        ...
        for map in map_order:
            ...
            while len(not_calculated) > 1:
                ...
                for map_tuple in map_dict[map]:
                    ...
                    fertiges, todo_tuple = combine_two_intervals_and_shift(
                        tuple, map_tuple, shifting_distance
                    )  # die map tuple dafür ausrechnen
                    fertige_tuple += [
                        x for x in fertiges if x not in fertige_tuple
                    ]  # die fertigen in fertig liste
                    not_calculated += [
                        x for x in todo_tuple if x not in not_calculated
                    ]  # die weiteren in not_calculated_wieder
                    counter += 1
...
def combine_two_intervals_and_shift(
    first: tuple, second: tuple, shifting_distance: int
):
    ...
    #
        ...
        right_tuple = (
            second[START] + shifting_distance,
            first[END] + shifting_distance,
        )  # right tuple has to be mapped
    ...
    # case 3:
        ...
        left_tuple = (
            first[START] + shifting_distance,
            second[END] + shifting_distance,
        )  # left tuple has to be mapped
    ...
    # case 4:
    elif (first[START] < second[START] and first[END] >= second[END]) or (
    ...
    # case 5:
        ...
        wurde_schon.append(
            [(first[START] + shifting_distance, first[END] + shifting_distance)]
...
def get_seeds(seed_line: str) -> list:
    ...
    for x in range(len(seed_list) // 2):
        range_tuple = (
            int(seed_list[2 * x]),
            int(seed_list[2 * x]) + int(seed_list[2 * x + 1]) - 1,
        )
